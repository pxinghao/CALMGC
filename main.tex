...
That is, if $\langle \tt{A}_{1,TS}, \dots, \tt{A}_{n,TS}, \tt{U}_{1,TS}, \dots, \tt{U}_{m,TS} \rangle$ and $\tt{B}_{TS}$ satisfies Merge GC Invariant \ref{inv:merge_gc} for $R$, then $\langle \tt{A}_{1,TS}^*, \dots, \tt{A}_{n,TS}^*, \tt{U}_{1,TS}^*, \dots, \tt{U}_{m,TS}^* \rangle = \tt{GC}(\tt{A}_{1,TS}, \dots, \tt{A}_{n,TS},$ $\tt{U}_{1,TS}, \dots, \tt{U}_{m,TS};$ $\tt{B}_{TS}, \tt{D}_{1,TS}, \dots, \tt{D}_{l,TS})$ and $\tt{B}_{TS}$ also satisfies the Merge GC Invariant \ref{inv:merge_gc}.
\end{property}
...
% \begin{align*}
% \tt{GCInd}_1(\tt{X}_{TS}; \tt{B}_{TS}) = \begin{cases}
% \emptyset & c\\
% d & e\\
% \end{cases}
% &&
...
Consider the join in Example \ref{ex:join_gc_no_punctuations}, now augmented with punctuation tables $\tt{P}_\tt{A}$ and $\tt{P}_\tt{C}$.
(If punctuations are not available, we can always let $\tt{P} = \emptyset$ with no loss of generality.)
...
It is a simple exercise to show that $\tt{GCJoin}$ tombstones a punctuation $p_a$ only when its set $p_a.\tt{A}$ is tombstoned (or does not exist yet), i.e., $(p_a \in \tt{P}_{\tt{A},\top}) \implies \tt{A}_\exists \cap p_a.\tt{A} = \emptyset$.
...
The rewritten GC program $\mathfrak{P}_{GC}$ maintains the Merge GC Invariant \ref{inv:merge_gc}.
\end{lemma}
\begin{proof}
We prove the lemma by an induction on $t$ and $i$.
...
Let $\widetilde{\tt{B}} = (\tt{B}_{\exists,t,0} \cup \tt{B}_{\top,t}) - (\tt{B}_{\exists,t-1,\iota} \cup \tt{B}_{\top,t-1})$, and observe that $\forall \langle \widehat{\tt{A}}_1, \dots, \widehat{\tt{A}}_n \rangle \geq \langle \tt{A}_{1,\exists,t,0}, \dots, \tt{A}_{n,\exists,t,0} \rangle \geq \langle \tt{A}_{1,\exists}^*, \dots, \tt{A}_{n,\exists}^* \rangle$,
...
\quad\tt{<-}\quad
\tt{GCI}(\tt{A}_E, \tt{C}_{1,E}, \dots, \tt{C}_{n,E}; \tt{B}_{1,E}, \dots, \tt{B}_{m,E}).
\end{align*}
...
Let $(\tt{A}_{1,E}^-, \dots, \tt{A}_{n,E}^-) = \tt{GCI}(\tt{A}_{1,E}, \dots, \tt{A}_{n,E}; \tt{B}_{1,E}, \dots, \tt{B}_{m,E})$.
The instantiated GC rule $\tt{GCI}$ is \emph{conservative} if $\forall \tt{A}_{1,E}, \dots, \tt{A}_{n,E}; \tt{B}_{1,E}, \dots, \tt{B}_{m,E}$, $\forall i=1,\dots,n$: $\tt{A}_{i,I}^- \subseteq \tt{A}_{i,I}$ and $\tt{A}_{i,!}^- = \pi_!(\tt{A}_{i,I})$.
\end{property}
In words, a conservative $\tt{GCI}$ only deletes tuples that it can observe.

\begin{property}[Instantiated GC Consistency]\label{property:instantiated_gc_consistent}
Suppose $\forall i=1,\dots,n$, $\tt{A}_{i,\exists} \subseteq \tt{A}_{i,I} \subseteq \tt{A}_{i,\exists} \cup \tt{A}_{i,\top}$, and $\forall i=1,\dots,m$, $\tt{B}_{i,\exists} \subseteq \tt{B}_{i,I} \subseteq \tt{B}_{i,\exists} \cup \tt{B}_{i,\top}$.
...
and 
$\tt{A}_{i,I}^- = \tt{A}_{i,\top}^*$ if $\tt{A}_i$ has primary keys.
\end{property}
...
\begin{example}[Copy GC]
$\tt{iGCCopy}(\tt{A}_I; \tt{B}_{1,I}, \dots, \tt{B}_{m,I}) = \tt{A}_I \cap \bigcap_{i=1}^m \tt{B}_{i,I}$ is consistent with $\tt{GCCopy}$.
\end{example}
Suppose
...
\begin{example}[Project GC]
$\tt{iGCProject}(\tt{A}_I; \tt{B}_I) = \{a \in \tt{A}_I ~:~ \pi_S(\{a\}) \subseteq \tt{B}_I\}$ is consistent with $\tt{GCProject}$.
\end{example}
...
\begin{example}[Select GC]
$\tt{iGCSelect}(\tt{A}_I; \tt{B}_I) = \sigma_{\neg P}(\tt{A}_I) \cup (\sigma_P(\tt{A}_I) \cap \tt{B}_I)$ is consistent with $\tt{GCSelect}$.
\end{example}
If $\tt{A}_I \subseteq \tt{A}_\exists \cup \tt{A}_\top$ and $\tt{B}_I \subseteq \tt{B}_\exists \cup \tt{B}_\top$, then
...
\begin{example}[Join with punctuations]
$\tt{iGCJoin}(\tt{A}_I, \tt{P}_{\tt{A},E}, \tt{C}_I, \tt{P}_{\tt{C},E}; \tt{B}_I) = \langle \tt{A}_I^-, \tt{P}_{\tt{A},E}^-, \tt{C}_I^-, \tt{P}_{\tt{C},E}^-\rangle$, where
\begin{align*}
...
\end{thm}
\begin{proof}
We will again prove by an induction on $t$ and $i$.
...
\end{align}
If $\tt{A}$ does not have primary keys, we apply the inductive hypothesis to \eqref{eq:thm:ins_correct:instant},
\begin{align*}
\tt{A}_{\exists,t,i}
...
By the consistency of $\tt{GCI}$, we have that $\tt{A}_I^- \subseteq \tt{A}_{T}^*$ if $\tt{A}$ has no primary keys, and $\tt{A}_I^- = \tt{A}_{T}^*$ otherwise.
Hence,
\begin{align}
&\tt{A}_{\exists,t,0}\nonumber\\
...
=&~ \tt{A}_{\exists,t,0} \cup \tt{A}_{\top,t} \nonumber
\end{align}
If $\tt{A}$ has primary keys, \eqref{eq:thm:inst_correct:timestep_0} and \eqref{eq:thm:inst_correct:timestep_1} are equalities, so $\tt{A}_{\exists,t,0} = \tt{A}_{I,t,0}$.
...
The proof of Theorem \ref{thm:instantiated_correctness} is largely unchanged, except with some careful handling of the differences between $\overline{\tt{B}_I}$ and $\underline{\tt{B}}$ resulting from ARM.
